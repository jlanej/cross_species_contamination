    ...
    if inputs.detect_summary is not None:
        ...
        if inputs.abs_detect_summary is not None:
            ...
            parts.append(
                ...
                "was <strong>not run</strong> for this report. "
                "Common causes:"
                "<ul>"
                "<li>No <code>taxa_matrix_abs.tsv</code> was available "
                "in the aggregate directory (re-run "
                "<code>csc-aggregate --idxstats …</code> with the "
                "per-sample <code>reads_summary.json</code> "
                "sidecars).</li>"
                "<li>Detection was invoked with "
                "<code>--no-abs-detection</code>.</li>"
                "<li>A <strong>pre-existing <code>detect/</code> "
                "directory was reused</strong> that pre-dates the "
                "absolute-burden matrix.  In this case "
                "<code>taxa_matrix_abs.tsv</code> exists in "
                "<code>aggregate/</code> but "
                "<code>detect/&lt;tier&gt;/abs/qc_summary.json</code> "
                "is missing.  Re-run <code>csc-detect</code> against "
                "the current <code>aggregate/</code> directory "
                "(re-aggregation is not required) so the "
                "<code>abs/</code> sub-directory is populated.</li>"
                "</ul>"
                "Without the absolute-burden side pass, outlier "
                "detection is purely compositional and may miss "
                "contamination episodes that are masked by "
